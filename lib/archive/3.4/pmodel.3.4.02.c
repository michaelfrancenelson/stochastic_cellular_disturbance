...
#include "../functions.3.4/random_normal.c" 
...
int main(int argc, char *argv[])
{
  ...
  int disturbRateMode;          // What is the mode number of disturbance patches per time step? 
  float disturbProbR;           // What is the probability of a disturbance event in a given time step?  (random model)
  float disturbSD;              // What is the standard deviation of the number of disturbance patches?
  ...
  disturbRateMode = atoi(readCfg(cfgFile, "disturbRateMode"));
  disturbProbR    = atof(readCfg(cfgFile, "disturbProbR"));
  disturbSD       = atof(readCfg(cfgFile, "disturbSD"));
  ...
  if(messages){                                                 // print out the parameters (if messages = 1):
    ...
    if(disturbYN){
      ...
      if(disturbModel){
        ...
        printf("mode disturbances/event     = %d\n", disturbRateMode);
        printf("disturbance SD              = %f\n", disturbSD);
  ...
  for(r = resumeStart; r <= resumeEnd; r++){                //  Loop through the reps of the simulation.
    ...
    for(day = timeStart; day <= timeMax; day++){                     // main program loop:
      ...
      if(disturbYN){                               // Call the habitat disturbance function.
        //printf("disturb\n");
        if(!disturbModel){
          if(day%distInterval == 0 & day != 0 & day >= distDelay){
            disturb(
              gridSizeX,
              gridSizeY,
        ...
        if(disturbModel){
          if(dsfmt_genrand_close_open(&dsfmt) <= disturbProbR & day >= distDelay) // If a random draw indicates disturbance:
            disturbR(
              gridSizeX,
              gridSizeY,
              n_species,
