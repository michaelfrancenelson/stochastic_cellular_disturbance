...
int main(int argc, char *argv[])
{
  ...
  int distCensusAll;                        // The number of disturbed cells (occupied or not) from disturb()
    ...
    if(count){                                                  // Write the census file.    
      ...
      if(timeStart == 0){
        ...
        for(i = 1; i < n_species + 1; i++){
          fprintf(censusOut, "%s", "sp_");
          fprintf(censusOut, "%d\t", i);
          fprintf(censusOut, "%s", "sp_");
          fprintf(censusOut, "%d", i);
          fprintf(censusOut, "%s\t", "_d");
          } // for(i)
    ...
    for(t = timeStart; t <= timeMax; t++){                     // main program loop:
      ...
      if(count){ 
        ...
        for(i = 1; i < n_species + 1; i++){
          ...
          fprintf(censusOut, "%ld\t", distCensusKilled[i - 1]);
          }
        fprintf(censusOut, "\n");
        fclose(censusOut);
