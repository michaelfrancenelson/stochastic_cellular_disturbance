...
September 2013:  Moved functions to individual files.

September 2013:  Changed random number generator function.

September 2013 to do:  make wrapping boundaries.
...
int main(int argc, char *argv[])
{
  ...
  int distType;                 // What shape for the disturbance patches?
  ...
  distType        = atoi(readCfg(cfgFile, "distType"));
  ...
  for(r = resumeStart; r <= resumeEnd; r++){                //  Loop through the reps of the simulation.
    ...
    for(t = timeStart; t <= timeMax; t++){                     // main program loop:
      ...
      for(i = 0; i < gridSize; i++)                        // Clear gridOut:
      for(j = 0; j < gridSize; j++)  
      ...
      if(dist){
        if(t%distInterval == 0) if(t != 0) disturb(        // Call the habitat disturbance function.  The disturbance function
