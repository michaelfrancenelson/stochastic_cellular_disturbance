...
int main(int argc, char *argv[])
{
  ...
  float         *displaceVector;                // A 1D array to hold the displacement probabilities
  ...
  displaceYN        = atoi(readCfg(cfgFile, "displace"));
  ...
  //displaceMatrix = calloc(n_species * n_resource, sizeof(long int));
  displaceVector = calloc(n_species * n_species * n_resource, sizeof(float));
  ...
  resourceGrid = calloc(gridSize, sizeof(char));        // Allocate memory for the reource and field grid arrays.
  fieldGrid    = calloc(2, sizeof(char));
    
  for(i = 0; i < gridSize; i++)                         // Create the 2D resource grid.
    resourceGrid[i] = calloc(gridSize, sizeof(char));

  for(i = 0; i < 3; i++){                               // Create the 3D field grid.
    fieldGrid[i] = calloc(gridSize, sizeof(char));
    for(j = 0; j < gridSize; j++)
      fieldGrid[i][j] = calloc(gridSize, sizeof(char));
    } // for(i)
  ...
  for(i = 0; i < n_species * n_resource; i++)               // Read the displacement probabilities and flatten them to a 1D array.
    for(j = 0; j < n_species; j++)
      fscanf(fileDisplaceMatrix, "%f", &displaceVector[i * n_species + j]);
  
  if(messages)                                              // Print the displacement probabilities.
    for(i = 0; i < n_species * n_resource; i++)
      for(j = 0; j < n_species; j++)
        printf("habitat %d:  species %d on species %d = %f\n", (i / n_species), i%n_species + 1, j + 1, displaceVector[n_species * i + j]);
  ...
  for(r = resumeStart; r <= resumeEnd; r++){                //  Loop through the reps of the simulation.
    ...
    for(t = timeStart; t <= timeMax; t++){                     // main program loop:
      ...
      if(save) if((t%saveInterval == 0) & (t >= saveDelay)) 
        saveArray(gridSize, fieldGrid[!(t%2)], directoryOut, filePrefix1 , t, r);
      ...
      if(save) if((t%saveInterval == 0) & (t >= saveDelay))
        saveArray(gridSize, fieldGrid[!(t%2)], directoryOut, filePrefix2 , t, r);
      } // for(t)
    }  // for(r)
  for(i = 1; i <= n_species; i++) 
    printf("%lu\n", speciesCounts[i]);
