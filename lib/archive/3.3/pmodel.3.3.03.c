...
#include "../functions/deathPanel.2.c"
...
int main(int argc, char *argv[])
{
  ...
  displaceVector = calloc(n_species * n_species * n_habitat, sizeof(float));
  ...
  habitatGrid = calloc(gridSize, ELEMENT_SIZE);        // Allocate memory for the reource and field grid arrays.
  ...
  for(i = 0; i < 3; i++){                               // Create the 3D field grid and 2D habitat grid.
    ...
    for(j = 0; j < gridSize; j++){
      ...
      habitatGrid[j] = calloc(gridSize, ELEMENT_SIZE);
      }
    } // for(i)
  ...
  for(i = 0; i < n_species * n_habitat; i++)               // Read the displacement probabilities and flatten them to a 1D array.
    for(j = 0; j < n_species; j++)
  ...
  for(r = resumeStart; r <= resumeEnd; r++){                //  Loop through the reps of the simulation.
    ...
    for(i = 0; i < gridSize; i++){                              // Read in the habitat and initial
