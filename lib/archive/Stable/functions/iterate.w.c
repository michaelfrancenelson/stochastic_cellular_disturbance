  ...
  int           j)
{
  ...
  for(k = 0; k < n_species; k++)                      // Initialize the species-specific neighbor counter to 0.
    ...
    for(l = j - 1; l <= j + 1; l++)  
      if(gridIn[(k + gridSize)%gridSize][(l + gridSize)%gridSize]){     // If neighboring cell gridIn[k][l] is occupied:
        neighbors[gridIn[(k + gridSize)%gridSize][(l + gridSize)%gridSize] - 1]++;
